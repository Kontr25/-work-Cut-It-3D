...
using UnityEngine;
using Joint = _Scripts.Joints.Joint;
...
namespace MeshSplitting.Splitables
{
    ...
    public class Splitable : MonoBehaviour, ISplitable
    {
        ...
        [SerializeField] private HingeJoint _hingeJointFirst;
        [SerializeField] private HingeJoint _hingeJointSecond;
        [SerializeField] private Joint _mainJointFirst;
        [SerializeField] private Joint _mainJointSecond;
        [SerializeField] private Rigidbody _rigidbody;
        [SerializeField] private ColorType splitableColorType;
        [SerializeField] private Material[] _materials;
        [SerializeField] private MeshRenderer _meshRenderer;
        private int _currentJointNumber = 0;
        ...
        public Rigidbody SplitableRigidbody
        {
            get => _rigidbody;
            set => _rigidbody = value;
        }

        public ColorType SplitableColorType => splitableColorType;
        ...
        private void CreateNewObjects()
        {
            ...
            for (int i = 0; i < newGOs.Length; i++)
            {
                Splitable splitable = newGOs[i].GetComponent<Splitable>();
                splitable.CheckJoint();
                splitable.SplitableRigidbody.isKinematic = false;
            }
        ...
        {
            StartCoroutine(CheckJointRoutine());
        }

        private IEnumerator CheckJointRoutine()
        {
            MeshCollider goCollider = transform.GetComponent<MeshCollider>();
            _currentJointNumber = 0;
            _mainJointFirst = null;
            _mainJointSecond = null;
            goCollider.enabled = false;
            goCollider.enabled = true;
            yield return new WaitForSeconds(.05f);
            if (_mainJointFirst == null)
            {
                if (_hingeJointFirst != null)
                {
                    Destroy(_hingeJointFirst);
                }
            }
            
            if (_mainJointSecond == null)
            {
                if (_hingeJointSecond != null)
                {
                    Destroy(_hingeJointSecond);
                }
            }
        }
        ...
        {
            if (other.TryGetComponent(out Joint joint))
            {
                if (_hingeJointFirst != null && joint.JointRigidbody == _hingeJointFirst.connectedBody)
                {
                    _mainJointFirst = joint;
                }
                if (_hingeJointSecond != null && joint.JointRigidbody == _hingeJointSecond.connectedBody)
                {
                    _mainJointSecond = joint;
                }
            }
        }
